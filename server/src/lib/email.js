...
const getTransporter = () => {
  // Check for the required OAuth2 variables instead of the old App Password
  if (!process.env.GMAIL_USER || !process.env.OAUTH_CLIENT_ID || !process.env.OAUTH_CLIENT_SECRET || !process.env.OAUTH_REFRESH_TOKEN) {
    throw new Error('Missing OAuth2 credentials. Ensure GMAIL_USER, OAUTH_CLIENT_ID, OAUTH_CLIENT_SECRET, and OAUTH_REFRESH_TOKEN are set.')
  }
...
export const sendOtpEmail = async ({ to, otp, purpose }) => {
  ...
  try {
    ...
    console.log(`OAuth2 OTP email sent successfully to ${to}`)
  } catch (err) {
    console.error('OTP email error:', {
      code: err.code,
      message: err.message
    ...
    if (err.message?.includes('invalid_grant')) {
      throw new Error('Google OAuth token expired or revoked. Please generate a new refresh token.')
    }

    throw new Error('Could not send OTP email. Check OAuth credentials and network access.')
  }
} 